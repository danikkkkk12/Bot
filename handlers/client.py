...
async def start_command(message: types.Message, user_id: int = 0):
    ...
    Если зарегистрирован, отправляем приветственное сообщение.
    """
...
async def get_language(query: Message | CallbackQuery, first: bool = False):
    ...
    prefix = f"sel_lang|{query.from_user.id}" if first else f"resel_lang|{q.from_user.id}"
    await query.answer(
        "Select language",
...
async def menu_output(callback: types.CallbackQuery):
    ...
    photo = types.FSInputFile("hello.jpg") if lang == "ru" else types.FSInputFile("hel.jpg")
...
# Обработчик для верификации через канал
...
async def channel_verification_handler(message: types.Message):
    """
    Обработчик для верификации пользователя через канал.
    """
...
# Обработчик для смены реферального кода
...
# Обработчик для ввода нового реферального кода
