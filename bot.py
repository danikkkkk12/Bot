...
# Словарь для хранения игровых данных
games = {}
...
def create_mines_field(size, mines):
...
def start(message):
    ...
    keyboard.add(InlineKeyboardButton("Launch", callback_data="launch_mines"))
    bot.send_message(message.chat.id, "Нажмите кнопку Launch, чтобы начать игру в Mines.", reply_markup=keyboard)

# Обработчик для кнопки "Launch"
@bot.callback_query_handler(func=lambda call: call.data == "launch_mines")
def launch_mines(call):
    keyboard = InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        InlineKeyboardButton("1 мина", callback_data="mines_1"),
    ...
    bot.send_message(call.message.chat.id, "Выберите количество мин:", reply_markup=keyboard)

# Обработчик для выбора количества мин
@bot.callback_query_handler(func=lambda call: call.data.startswith("mines_"))
def select_mines(call):
    mines = int(call.data.split("_")[1])  # Получаем количество мин
    size = 5  # Размер поля (5x5)
    field = create_mines_field(size, mines)
    revealed = [[False for _ in range(size)] for _ in range(size)]
    games[call.message.chat.id] = {"field": field, "revealed": revealed, "mines": mines}
    
    # Отправляем начальное поле
    bot.send_message(call.message.chat.id, "Игра началась! Откройте клетку:")
    send_field(call.message.chat.id)

# Функция для отправки игрового поля
def send_field(chat_id):
    game = games.get(chat_id)
    if game:
        field = game["field"]
        revealed = game["revealed"]
        text = display_field(field, revealed)
        keyboard = create_field_keyboard(revealed)
        bot.send_message(chat_id, text, reply_markup=keyboard)

# Функция для создания клавиатуры с полем
def create_field_keyboard(revealed):
    size = len(revealed)
    ...
    for i in range(size):
        ...
        for j in range(size):
            if revealed[i][j]:
                row.append(InlineKeyboardButton("✅", callback_data=f"cell_{i}_{j}"))
            else:
                row.append(InlineKeyboardButton("⬜️", callback_data=f"cell_{i}_{j}"))
        keyboard.add(*row)
    return keyboard
...
def click_cell(call):
    ...
    if game:
        ...
        if field[x][y] == "💣":
            bot.send_message(chat_id, "💥 Вы наткнулись на мину! Игра окончена.")
            games.pop(chat_id)
        else:
            revealed[x][y] = True
            if all(all(revealed[i][j] or field[i][j] == "💣" for j in range(len(field))) for i in range(len(field))):
